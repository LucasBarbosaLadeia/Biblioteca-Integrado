...
const UserCard = ({ name, registration, email, phone }) => {
  return (
    <LinearGradient
      colors={["#420a3cff", "#302a63ff"]} // ­ƒöÑ gradiente igual ao da imagem
      start={{ x: 0, y: 0 }}
      end={{ x: 1, y: 1 }}
    ...
    >
      <View style={styles.row}>
        ...
        <View style={styles.info}>
          ...
          <Text style={styles.meta}>RA: {maskRA(registration) || "ÔÇö"}</Text>

          <View style={styles.line}>
            <Ionicons name="mail-outline" size={14} color="#c6d1e6" />
            <Text style={styles.metaText}>{email || "ÔÇö"}</Text>
          </View>

          <View style={styles.line}>
            <Ionicons name="call-outline" size={14} color="#c6d1e6" />
            <Text style={styles.metaText}>{phone || "ÔÇö"}</Text>
          </View>
        </View>
      </View>
    </LinearGradient>
